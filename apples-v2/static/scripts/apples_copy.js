...
document.addEventListener('DOMContentLoaded', () => {
    ...
    *****************************************************/
    ...
    socket.on('update_game', data => {
        ...
        if(data.winner_id) {
            if(Player.list[winner_id]){
                Player.list[winner_id].winner = true;
            }
        }
    });
    ...
    *****************************************************/
    ...
    function updateGame(state, players, green_card) {
        for (var i = 0; i < players.length; i++){
            ...
            if (gameState != state || ((p.id == selfId) && (p.type != data.player_type))) {
    ...
    *****************************************************/
    ...
    /*
    document.querySelectorAll(".red_card").addEventListener('onmousedown', function() {
        selectedCard = 
    })*/
    ...
    function sendSubmission(){
        ...
        if(selected.classList.contains('red_card')){
            var item = selected.firstElementChild;
            var msg = {
                'card_item': item,
